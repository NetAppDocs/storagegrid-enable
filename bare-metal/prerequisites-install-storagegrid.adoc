...
In this example, the disk sizes shown in the above table are needed per container type. The requirements per physical host are described in link:prerequisites-install-storagegrid.html#physical-host-layout-and-requirements[Physical host layout and requirements], later in this document.
