...
. Open the StorageGRID Grid Manager.
. In top right corner, select the Question mark then API Documentation.
. In top right corner, click on the Private API Documentation page link.
. Expand ilm-advanced.
. Select GET ilm-advanced.
. Select Try it out, then Execute.
. Check the response result.
.. If the values are null, then it means the default ilm-advanced values are in-use.
.. If the values are not null, then it means custom ILM advanced values are in-use. Copy all the output after "data" :, starting with the { up untill the second to last }.
... Save it in some text editor.
+
Example response:
+
image:veeam-bp/get_ilm_adv_before.png[GET ILM-Advanced output]
+
. Select PUT ilm-advanced.
. Select Try it out to begin editing the API body.
.. By default, the API body will contain default values and not any custom values that were previously configured. This is the reason it is VERY important to execute steps 5-7.
. If non-default values are found in step 5-7, then replace the API body with the output saved in step 7. . Otherwise if the values were null in step 5-7, then leave API body as is.
. Adjust the following parameters in the API body box:
.. Set the synchronous value to false.
+
Example API body text:
+
image:veeam-bp/put_ilm_adv.png[PUT ILM-Advanced intput]
+
. Once complete, select Execute
...
* link:https://docs.netapp.com/us-en/storagegrid/[NetApp StorageGRID Product Documentation^]
