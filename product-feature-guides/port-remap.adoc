...
. Create the directory to copy all audit log files to a temporary location on a separate grid node: `ssh admin@_grid_node_IP_`
...
. Copy all audit log files: `scp * admin@_grid_node_IP_:/var/local/tmp/saved-audit-logs`
...
. Create the directory to copy the prometheus data to a temporary location on a separate grid node: 
 .. Log in to the node:
  ... Enter the following command: `ssh admin@_grid_node_IP_`
  ... Enter the password listed in the `Passwords.txt` file.
...
. From the Admin Node, stop the Prometheus service: `service prometheus stop`
 .. Copy the Prometheus database from the source Admin Node to the other node backup location Node: `/rsync -azh --stats "/var/local/mysql_ibdata/prometheus/data" "_destination_node_:/var/local/tmp/prometheus/"`
...
. Copy the mysql dump file to an alternat node: `scp /var/local/tmp/mysql-mi.sql grid_node_IP_:/var/local/tmp/mysql-mi.sql`
