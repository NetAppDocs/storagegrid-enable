...
If you prefer to configure your load balancer for pass through, you must install the certificate on StorageGRID. menu:Go to Configuration[Server Certificates > Object Storage API Service Endpoints Server Certificate].
...
* *UI Method.* menu:Go to Support[Metrics > S3 Overview > LDR HTTP Sessions]
