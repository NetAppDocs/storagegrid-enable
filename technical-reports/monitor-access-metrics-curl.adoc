...
----
+
Now you can use the authorization token string to access metrics through cURL. The process to access metrics is similar to the steps in section link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api[Advanced monitoring in StorageGRID]. However, for demonstration purposes, we show an example with GET /grid/metric-labels/{label}/values selected in the Metrics category.
