...
Audit logs can be exported by syslog allowing audit messages to be mined by tools such as Splunk and ELK. There are four types of audit messages:
...
Audit logs can be stored in a S3 bucket for long term retention and application access.
